...
partitionByFstM f (Fold stepL beginL doneL) (Fold stepR beginR doneR) =
    ...
    begin = do
        ...
        case resL of
            Partial sL ->
                case resR of
                    Partial sR -> return $ Partial $ RunBoth sL sR
                    Done bR -> return $ Partial $ RunLeft sL bR
            Done bL -> do
                case resR of
                    Partial sR -> do
                        bR <- doneR sR
    ...
    step (RunBoth sL sR) a = do
        ...
        case r of
            Left b -> do
                res <- stepL sL b
                case res of
                    Partial sres -> return $ Partial $ RunBoth sres sR
                    Done bL -> do
                        bR <- doneR sR
                        return $ Done (bL, bR)
    ...
    step (RunBoth sL sR) a = do
        ...
        case r of
            ...
            Right c -> do
                res <- stepR sR c
                case res of
                        Partial sres -> return $ Partial $ RunBoth sL sres
                        Done bR -> do
                            bL <- doneL sL
                            return $ Done (bL, bR)

    step (RunLeft _ _) _ = error "Invalid options"

    step (RunRight _ _) _ = error "Invalid options"

    done (RunBoth sL sR) = (,) <$> doneL sL <*> doneR sR

    done (RunLeft _ _) = error "Invalid options"

    done (RunRight _ _) = error "Invalid options"
...
unzipWithFstM f (Fold stepL beginL doneL) (Fold stepR beginR doneR) =
    ...
    step (RunBoth sL sR) a = do
        ...
        case resL of
            Partial sresL ->
                case resR of
                          Partial sresR -> return $ Partial $ RunBoth sresL sresR
                          Done bresR -> return $ Partial $ RunLeft sresL bresR
            Done bL -> do
                case resR of
                    Partial sresR -> do
                        bR <- doneR sresR
